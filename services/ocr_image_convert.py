...
def add_padding(image, padding=10, color=255):
    h, w = image.shape
    padded = np.full((h + 2 * padding, w + 2 * padding), color, dtype=np.uint8)
...
def preprocess_image(img):
    ...
    if img.shape[-1] == 4:
        ...
        img = (
            img[:, :, :3] * alpha_factor + white_background * (1 - alpha_factor)
        ).astype(np.uint8)
    ...
    # 이미지 크기 조정
    resized = cv2.resize(
        gray, (0, 0), fx=RESIZE_FACTOR, fy=RESIZE_FACTOR, interpolation=cv2.INTER_AREA
    )
